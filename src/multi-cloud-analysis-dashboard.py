...
#             elif term_type.lower() == 'ondemand':
#                 for term_id, term in fetched_raw_terms[term_type].items():
#                     for dim in term.get('priceDimensions', {}).values():
#                         price = float(dim['pricePerUnit'].get('USD', 0.0))
#                         prices['AWS OnDemand'] = round(price * 730, 2)
...
#     global pricing_df
#     new_df = pd.DataFrame(rows)
#     if 'pricing_df' not in globals() or pricing_df is None:
#         pricing_df = new_df
#     else:
#         pricing_df = pd.concat([pricing_df, new_df], ignore_index=True).drop_duplicates(subset=["Model", "Cloud", "Region"])
...
#         df = pricing_df
#     else:
#         df = pricing_df[pricing_df["Pricing Type"].str.strip().str.lower() == selection.strip().lower()]
...
#     p = figure(x_range=list(df["Model"]), height=350, title=f"{selection} Pricing Across Clouds", tools="hover", tooltips=[
#         ("Model", "@models"),
#         ("Cloud", "@cloud"),
#         ("Region", "@region"),
#         ("Monthly", "@prices{$0.00}")
